...
Public Class AccessBuildCSharp
    ...
    Shared Sub buildClass(ByVal table As Table, targetPath As String, changed As Boolean, properties As Boolean)
        ...
        Using fs As New StreamWriter(String.Format("{0}\{1}.cs", targetPath, className, False))
            ...
            If changed Then
                ...
                tw.WriteLine()
            End If
            tw.WriteLine(String.Format("{0}static string TABLE = ""{1}"";", vbTab, table.getName))
            tw.WriteLine()
            ...
            For Each col As columns In cols
                If Not col.getIsAutoNumber Then
                    If convertDBType(col).Contains("string") Then
                        tw.Write("'?'")
                    ElseIf col.getDbType.Contains("DateTime") Then
                        tw.Write("'?'")
                    Else
                        tw.Write("?")
                    End If
            ...
            For Each col As columns In cols
                If Not col.getIsAutoNumber Then
                    tw.WriteLine(String.Format("{0}{0}{0}""["" + {1}{2} + ""]={3}{4}"" +", vbTab, COL_PREFIX, Main.setConstant(col.getName.Replace(" ", "_").Replace("-", "_").Substring(2)).ToUpper, IIf(convertDBType(col).Contains("String"), "'?'", IIf(convertDBType(col).Contains("Date"), "'?'", "?")), IIf(n < c - 1, ",", "")))
                    n += 1
                End If
            Next
            tw.WriteLine(String.Format("{0}{0}{0}"" WHERE "" + {1};", vbTab, key))
            ...
            ' save Statement
            frm.lst.Items.Add("  save Statement")
            ...
            For Each col As columns In cols
                ...
                If Not col.getIsAutoNumber Then
                    tw.WriteLine("{0}{0}set{{ {1} = value;", vbTab, col.getName.Replace(" ", "").Replace("-", ""))
